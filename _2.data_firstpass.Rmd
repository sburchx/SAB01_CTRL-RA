...
ggsave(bargraph.all, path = "03_plots",
      filename = str_c("all.bargraph",  format(Sys.Date(), "%Y-%m-%d"), format(Sys.time(), "_%H%M"), ".pdf"),
...
#Combined FACETED graphs
(1) facet by treatment to compare T21vD21 
(2) facet by cell to compare treatment 
...
for (measure in measurements) {
  ...
  p <- ggplot(measurement_data, aes(x = factor(day), y = mean, fill = treatment, color= treatment)) +
    ...
    scale_alpha_manual(values = c("S3D21" = 0.8, "S3T21" = 0)) +
...
#COMBINED GRAPH - D21 + T21 multiple treatments
```{r filter}
#filtering data for days across treatment, not always necessary 
...
#for this there are two treatments present CTRL and RA. 
```{r generating graphs}
...
summary_data <- sumstats_all %>% filter(measurement == "tot_isl")
measure_data <- long_data %>% filter(measurement == "tot_isl")

summary_data <- summary_data %>% filter(day %in% c("4", "6", "8", "10"))
measure_data <- measure_data %>% filter(day %in% c("4", "6", "8", "10"))
...
ggplot(summary_data, aes(x = factor(day), y = mean, fill = treatment, color = treatment)) +
  
  # Bars with consistent position dodge and group interaction
  geom_bar(stat = "identity", position = position_dodge(width = 0.8), 
           width = 0.7, size = 1, aes(alpha = cell, group = interaction(treatment, cell))) +
  scale_y_continuous(limits = c(0, 100), labels = scales::percent_format(,1)) +
  ...
  # Error bars with the same position dodge and group interaction
  geom_errorbar(aes(ymin = mean - sd, ymax = mean + sd, group = interaction(treatment, cell)), 
                width = 0.4, size = 0.7, position = position_dodge(width = 0.8), color = "black") +

  # Data points for individual measurements with the same group interaction
  geom_point(data = measure_data, aes(x = factor(day), y = value, group = interaction(treatment, cell)), 
             position = position_jitterdodge(jitter.width = 0.1, dodge.width = 0.8), 
             size = 1.5, alpha = 0.8, color = "black", show.legend = FALSE) +
  ...
  guides(
    ...
    alpha = "none"  # Hide alpha legend if not needed as a separate entry
  ) +
...
ggsave(filename = paste0("combined-all_islet.pdf"), path = "03_plots",
